<?php
    ...
    if(!$acc){
        ...
        $trainObj = new Admin;
        $stations = $trainObj->getAllStationAsOptions(); 
        $result = $trainObj->getParticularTrainDetails($acc);
        if(!$result){
            echo "<main class='main-content bgc-grey-100'><div class='alert alert-danger' role='alert' style='vertical-align:middle'>
            Error Occured. Must specify Station id, short or name Correctly!. 
            </div><main>";
            ...
            if (isset($_POST['addDetail'])){
                ...
                $_SESSION['updated'] = $result->updateTrain($acc, $trainName, $goStations, $goStationTimes, $returnStations, $returnStationTimes, $gostationpricesarr, $returnstationpricesarr);
                header("Refresh:");
            }
?>
<!-- ### $App Screen Content ### -->
...
<?php
    if(@$_SESSION['updated'] == "Updated"){
        echo '<div class="alert alert-success" role="alert">Successfully Updated! To see Changes Please Refresh Your Page</div>';
            ...
            <form class="container" id="needs-validation" novalidate method="post" action="#"> 
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="text" class="form-control" id="validationCustom01" name="acc" placeholder="Train ID Eg:- 1N40001 " value="<?php echo $acc; ?>" required disabled>
                ...
                <div class="col-md-6 mb-3">
                  ...
                  <input type="text" class="form-control" id="validationCustom02" name="train" placeholder="Eg: - Yaldevi Long Express" required value="<?php echo $result->getTrainName(); ?>">
                  <div class="invalid-feedback">
                ...
                <div class="goStations col-md-6 mb-3">
                    ...
                    <?php
                        foreach ($result->getGoSchedule() as $value) {
                            echo "<div class='form-row'><div class='form-group col-md-3'>
                            <label for='inputState'>Station</label>
                            <select class='form-control' required name='gostationsarr[]'><option value='".$value[0]."' selected hidden>".$value[0]."</option>$stations</select></div>
                            <div class='form-group col-md-3'><label >Time</label><input type='time' class='form-control' name='gostationtimesarr[]' value='".$value[1]."' required></div>
                            <div class='form-group col-md-4'><label >Ticket Price</label><input type='text' class='form-control' name='gostationpricesarr[]' placeholder='Enter R.s serperate by ,' title='Insert amount serperate by comma' value='".$value[2]."'></div>
                            <a href='javascript:void(0);' class='remove_button col-md-2' style=float:right >&nbsp; X </a></div>";
                        }
                    ?>
                </div>
                ...
                <div class="col-md-12 mb-6">
                 ...
                 <div class="returnStations col-md-6 mb-3">
                    ...
                    <?php
                        foreach ($result->getReturnsSchedule() as $value1) {
                            echo "<div class='form-row'><div class='form-group col-md-3'>
                            <label for='inputState'>Station</label>
                            <select class='form-control' required name='returnstationsarr[]'><option value='".$value1[0]."' selected hidden>".$value1[0]."</option>$stations</select></div>
                            <div class='form-group col-md-3'><label >Time</label><input type='time' class='form-control' name='returnstationsimesarr[]' value='".$value1[1]."' required></div>
                            <div class='form-group col-md-4'><label >Ticket Price</label><input type='text' class='form-control' name='returnstationpricesarr[]' placeholder='Enter R.s serperate by ,' title='Insert amount serperate by comma' value='".$value1[2]."'></div>
                            <a href='javascript:void(0);' class='remove_button col-md-2' style=float:right >&nbsp; X </a></div>";
                        }
                    ?>
                </div>
              ...
              <button class="btn btn-primary" type="submit" name="addDetail">Update Train</button>
